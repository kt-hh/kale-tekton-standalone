...
def compile_pipeline(pipeline_source: str, pipeline_name: str) -> str:
    ...
    with open(pipeline_package, 'r') as pipelinerun:
        pipeline_dict = yaml.safe_load(pipelinerun)
    pipeline_dict['metadata']['namespace'] = podutils.get_namespace()
    pipeline_dict['spec']['serviceAccountName'] = 'default-editor'

    with open(pipeline_package, 'w') as pipelinerun:
        yaml.dump(pipeline_dict, pipelinerun)
...
def run_pipeline(pipeline_name: str) -> Any:
    log.info("Submitting Tekton PipelineRun '%s'...", pipeline_name)
    with open(pipeline_name + '.pipeline.yaml') as f:
